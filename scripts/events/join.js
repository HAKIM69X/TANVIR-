...
module.exports.run = async function({ api, event, Users }) {
  ...
  if (event.logMessageData.addedParticipants && Array.isArray(event.logMessageData.addedParticipants) && event.logMessageData.addedParticipants.some(i => i.userFbId == api.getCurrentUserID())) {
    ...
    let gifUrl = 'https://imgur.com/a/W7Hlieg';
...
.then(response => {
  ...
  if (event.logMessageData.addedParticipants.some(i => i.userFbId == api.getCurrentUserID())) {
    ...
    return api.sendMessage("আমাকে কেউ দেখেনা☹️", event.threadID, () => api.sendMessage({ body: `কেডা এড দিছোস রে....`, attachment: fs.createReadStream(gifPath)}, threadID));
  }})
.catch(error => {
    console.error(error);
});
    ...
    try {
      ...
      (typeof threadData.customJoin == "undefined") ? msg = ` হ্যালো {name} 🖤\nতোমাকে {threadName} গ্রুপে স্বাগতম😌\nতুমি গ্রুপের {soThanhVien} সদস্য, Enjoy✨🎊"\n─────────────────\n[ {time} - {thu} ]` : msg = threadData.customJoin;
